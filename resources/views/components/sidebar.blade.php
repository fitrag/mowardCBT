    ...
    <div class="flex grow flex-col gap-y-5 overflow-y-auto rounded-2xl bg-white px-6 pb-4 shadow-[0_0_40px_-10px_rgba(0,0,0,0.05)] ring-1 ring-slate-900/5 custom-scrollbar">
        ...
        <div class="flex h-24 shrink-0 items-center">
            <div class="flex items-center gap-x-3">
                ...
                @endphp
                <div class="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center shadow-lg shadow-indigo-500/30 overflow-hidden">
                    @if($appLogo)
                        <img src="{{ asset('storage/' . $appLogo) }}" alt="Logo" class="h-full w-full object-cover">
                    @else
                ...
                <div class="flex flex-col">
                    <span class="text-lg font-bold text-slate-900 tracking-tight leading-none">{{ $appName }}</span>
                ...
                <li>
                    ...
                    <ul role="list" class="-mx-2 space-y-2">
                        @if(auth()->user()->role->value === 'admin')
                            ...
                            <x-sidebar-dropdown label="Test" activePattern="tests.*|tokens.*|reports.*">
                                <x-slot name="icon">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                                </x-slot>
                                <x-sidebar-sublink route="tests.index" activePattern="tests.*" label="Tests" />
                                <x-sidebar-sublink route="tokens.index" activePattern="tokens.*" label="Tokens" />
                                <x-sidebar-sublink route="reports.test-results" activePattern="reports.*" label="Reports" />
                            </x-sidebar-dropdown>
                            <li>
                                <x-sidebar-link route="settings.index" activePattern="settings.*" label="Settings">
                                    <x-slot name="icon">
                        ...
                        @else
                            <!-- Student Menu -->
                            <li>
                                <x-sidebar-link route="student.dashboard" label="Dashboard">
                                    <x-slot name="icon">
